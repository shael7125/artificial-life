...
def mutate_robot(robot, max_attempts=50):
    """
    Mutates a robot by flipping one voxel in its mask while preserving connectivity.

    Mutation rules:
    - Adding a voxel (0→1) is allowed only if it touches the robot.
    - Removing a voxel (1→0) is allowed only if the robot remains connected.
    - Never allow the robot to become empty.
    """
    ...
    for _ in range(max_attempts):
        ...
        r = np.random.randint(0, MASK_DIM)
        c = np.random.randint(0, MASK_DIM)

        # ---------- CASE 1: ADD VOXEL ----------
        if mask[r, c] == 0:

            # check 4-neighborhood connectivity
            neighbors = [
                (r-1, c),
                (r+1, c),
                (r, c-1),
            ...
            touches_structure = False
            for nr, nc in neighbors:
                if 0 <= nr < MASK_DIM and 0 <= nc < MASK_DIM:
                    if mask[nr, nc] == 1:
        ...
        # ---------- CASE 2: REMOVE VOXEL ----------
            ...
            if mask.sum() <= 1:
                continue  # don't delete the last voxel

            test_mask = mask.copy()
            test_mask[r, c] = 0

            labeled, num_features = ndimage.label(test_mask)

            if num_features != 1:
                continue  # removal disconnects robot

            mask = test_mask
            break

    # ----- cleanup: keep largest component & shift bottom-left -----
    labeled, num_features = ndimage.label(mask)
    component_sizes = ndimage.sum(mask, labeled, range(1, num_features + 1))
    largest_component = np.argmax(component_sizes) + 1
    mask = (labeled == largest_component)
    ...
    new_mask = np.zeros((MASK_DIM, MASK_DIM), dtype=int)
    h, w = component.shape
    new_mask[MASK_DIM - h:MASK_DIM, 0:w] = component
    mask = new_mask

    # ----- convert mask to robot geometry -----
    masses, springs = mask_to_robot(mask)
    masses = masses * SCALE
